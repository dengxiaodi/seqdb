...
function get_sampleinfo($sampleid=0){
    if(is_numeric($sampleid)){
        $id = M('sample')->where('sampleid='.$sampleid)->getField('id');
        if($id){
            $url = U('sample/detail','sid='.$id);
        }else{
            $url = "###";
        }   
        return $url;
...
function get_libinfo($libid=0){
    if(is_numeric($libid)){
        $id = M('library')->where('lib_id='.$libid)->getField('id');
        if($id){
            $url = U('library/detail','sid='.$id);
        }else{
            $url = "###";
        }   
        return $url;
...
function get_seqinfo($seqid=0){
    if(is_numeric($seqid)){
        $id = M('seq')->where('seq_id='.$seqid)->getField('id');
        if($id){
            $url = U('seq/detail','sid='.$id);
        }else{
            $url = "###";
        }   
        return $url;
...
function get_seqlib_count($seqid=0){
    $count = M('seqlib')->where('seq_id='.$seqid)->count("lib_id");
    return $count;
}

function get_libid($libid=0){
